...
class AbstractModel(nn.Module):
    def _initialize_weights(self):
        print(" _initialize_weights(self):")
        # print("self.modules() in AbstractModel unet",self.modules().shape)
    ...
    def initialize_encoder(self, model, model_url, num_channels_changed=False):
        print("initialize_encoder(self, model, model_url, num_channels_changed=False):")
...
class EncoderDecoder(AbstractModel):
    # super().__init__(num_classes=1, num_channels = 4, encoder_name = backbone_arch = 'resnet34')
    def __init__(self, num_classes, num_channels=3, encoder_name='resnet34'):
        if not hasattr(self, 'first_layer_stride_two'): #has been made true in resnet(EncoderDecoder)
        ...
        self.bottlenecks = nn.ModuleList(
            [
                self.bottleneck_type(self.filters[-i - 2] + f, f) for i, f in enumerate(reversed(self.decoder_filters[:]))
            ]
        )
...
class ConvBottleneck(nn.Module):
    def __init__(self, in_channels, out_channels):
        print("ConvBottleneck(nn.Module): in_channels, out_channels", in_channels, out_channels)
        super().__init__()
...
class UnetDecoderBlock(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels):
        print("UnetDecoderBlock(nn.Module): in_channels, middle_channels, out_channels",in_channels, middle_channels, out_channels)
        # exit()
        super().__init__()
...
class Resnet(EncoderDecoder):
    # super().__init__(seg_classes=1, backbone_arch=backbone_arch = 'resnet34')
...
class ResnetSuperVision(Resnet):
    # ResnetSuperVision(1, backbone_arch='resnet34')
    def __init__(self, seg_classes, backbone_arch):
